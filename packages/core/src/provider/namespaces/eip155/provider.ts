 ...
 * EIP-155 Provider
 *
 * EIP-1193 兼容的以太坊 Provider 实现
 */
import type { JsonRpcVersion2 } from "@metamask/utils";
import { EventEmitter } from "eventemitter3";
import { evmProviderErrors, evmRpcErrors } from "../../errors";
import type { EIP1193Provider, EIP1193ProviderRpcError, RequestArguments } from "../../types/eip1193";
import type { Transport, TransportMeta, TransportState } from "../../types/transport";
import { isTransportMeta } from "../../utils/transportMeta";
import {
...
import { Eip155ProviderState, type ProviderPatch, type ProviderSnapshot, type ProviderStateSnapshot } from "./state";
...
export class Eip155Provider extends EventEmitter implements EIP1193Provider {
	...
	send = (methodOrPayload: string | LegacyPayload, paramsOrCallback?: unknown) => {
		if (typeof methodOrPayload === "string") {
			...
			return this.request(requestArgs).then((result) => ({ id: undefined, jsonrpc: "2.0" as const, result }));
		}
	...
	#startConnect() {
		...
		this.#connectInFlight = this.#transport
			.connect()
			.catch(() => {
				// Best-effort: readiness 由 timeout 保证
			})
			.finally(() => {
